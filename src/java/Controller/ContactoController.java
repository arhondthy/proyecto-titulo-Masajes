...
public class ContactoController implements Serializable {
    ...
    @EJB
    private Facade.ContactoFacade ejbFacade;
    ...
    private ContactoFacade getFacade() {
        return ejbFacade;
         ...
         if(nuevo!=null){
            Boolean resp = ejbFacade.guardar(nuevo);
            if (resp) {
                nuevo=null;
                RequestContext.getCurrentInstance().update("MensajeErrorContacto");
                FacesContext context = FacesContext.getCurrentInstance();
                context.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO, "Exito", Constant.CONTACTO_EXITO));
            } else {
                nuevo=null;
                RequestContext.getCurrentInstance().update("MensajeErrorContacto");
                FacesContext context = FacesContext.getCurrentInstance();
                context.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "Error", Constant.CONTACTO_ERROR));
            }
         }
    ...
    public void setNuevo(Contacto nuevo) {
        this.nuevo = nuevo;
    }
